...
function parseAuthResponse(data) {
  ...
  // D-LITE frontend expects `user.username` for display. OAuth (Google) uses metadata fields.
...
export async function loginWithGoogle() {
  ...
  // Client redirects via window.location.assign(data.url); session arrives on /auth/callback.
...
/** @param {(user: object | null, event?: string) => void} handler — event matches Supabase auth (e.g. SIGNED_IN). */
export function subscribeToAuthState(handler) {
  if (!supabase) {
    handler(null)
    return () => undefined
  }
  // Supabase emits INITIAL_SESSION on subscribe, then SIGNED_IN / TOKEN_REFRESHED / SIGNED_OUT / etc.
